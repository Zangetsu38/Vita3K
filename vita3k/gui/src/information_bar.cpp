...
void draw_information_bar(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto DEFAULT_BAR_COLOR = gui.vita_area.friends || gui.vita_area.friend_profile ? DEFAULT_INDICATOR_WHITE_COLOR : DEFAULT_INDICATOR_BLACK_COLOR;
    const auto DEFAULT_INDICATOR_COLOR = gui.vita_area.friends || gui.vita_area.friend_profile ? DEFAULT_INDICATOR_BLACK_COLOR : DEFAULT_INDICATOR_WHITE_COLOR;

    const auto BAR_COLOR = is_theme_color ? gui.information_bar_color.bar : DEFAULT_BAR_COLOR;
    const auto INDICATOR_COLOR = is_theme_color ? gui.information_bar_color.indicator : DEFAULT_INDICATOR_COLOR;
    ...
    if (emuenv.display.imgui_render && !gui.vita_area.start_screen && !gui.vita_area.user_management && !gui.configuration_menu.settings_dialog && !gui.configuration_menu.custom_settings_dialog && !gui.help_menu.vita3k_update && get_sys_apps_state(gui) && (ImGui::IsWindowHovered(ImGuiHoveredFlags_None) || ImGui::IsItemClicked(0)))
