...
static void save_themes_cache(EmuEnvState &emuenv) {
...
static uint32_t current_menu_size = 4;
...
void draw_settings(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const std::map<uint32_t, std::string> settings_list{
        { SettingsMenu::SOUND_DISPLAY, sound_display["title"] },
        ...
        case Menu::THEME: {
            ...
            if (selected.empty()) {
                ...
                case Popup::DEL: {
                    ...
                    ImGui::SetNextWindowPos(ImVec2(WINDOW_POS.x + (WINDOW_SIZE.x - POPUP_SIZE.x) / 2.f, WINDOW_POS.y + ((WINDOW_SIZE.y - POPUP_SIZE.y) / 2.f)), ImGuiCond_Always);
