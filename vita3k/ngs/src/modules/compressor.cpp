...
void CompressorModule::set_default_preset(const MemState &mem, ModuleData &data) {
    auto *params = data.info.data.cast<SceNgsCompressorParams>().get(mem);
    if (!params) {
        return;
    }
    ...
    *params = {};
    ...
    params->desc.size = sizeof(SceNgsCompressorParams);
    params->fRatio = 1.0f;
}
