...
static GLuint load_texture(int32_t width, int32_t height, const unsigned char *data) {
    GLuint texture;
...
static void init_icons(HostState &host) {
    for (Game &game : host.gui.game_selector.games) {
        ...
        host.gui.game_selector.icons[game.title_id].init(load_texture(width, height, data), glDeleteTextures);
        delete data;
    }
}
