        ...
        if ((dp = readdir(d)) != NULL) {
            ...
            if ((strcmp(d_name_utf8.c_str(), ".")) && (strcmp(d_name_utf8.c_str(), ".."))) {
                ...
                if (vfs::read_app_file(params, state.pref_path, state.io.title_id, "sce_sys/param.sfo")) {
                    ...
                    find_data(state.game_icon, sfo_handle, "CATEGORY"); // Test only, need remplace by inject icon
                    ...
                    state.gui.game_selector.games.push_back({ state.game_icon, state.game_version, state.game_title, state.io.title_id });
                }
            }
        }
    } while (dp);
